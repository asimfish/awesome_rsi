...
4. **成本未充分展开**：每步 N 份诊断 + N 条精炼 + 双模型更新，比 GRPO 高数倍，论文缺 token/算力对比表——RQGM 有"1.35× 更少 token"的核算，ECHO 没有。
5. **WGtG 方法论警告**："共进化 judge 与 policy、报告 policy 涨分"正是 WGtG 证明无法验证评估器有效性的实验设计——ECHO 的 critic 有效性依赖 R 的可信度，而 R 未被审计。
