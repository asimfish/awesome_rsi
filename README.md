...
这是一个关于 Recursive Self-Improvement（递归自改进）的论文列表和调研仓库，覆盖从 1965 年 Good 的智能爆炸猜想到 2026 年的自进化 agent 研究。除论文链接外，每篇核心论文都配有中文解读和保留原版式的中文翻译 PDF。

调研的主要结论：目前能稳定运行的自进化系统，都在进化循环之外保留了一个固定的评估依据（人工标注集、冻结的验证器、回归测试等，下文统称"锚"）。去掉它之后，评估器会被优化压力带偏，而且从任务分数上看不出来。所以这类系统的瓶颈不在执行，而在怎样低成本地获得可靠的评估。
...
_Lilian Weng_ — 把 auto-research、自改进 agent、进化式程序搜索三条研究线放在一个框架下：harness（模型外围负责编排、工具调用、上下文管理和评估的代码）可以像其他代码一样被搜索和优化。给出优化对象从 prompt、上下文、工作流、harness 代码到优化器代码的递进顺序，并列出七个未解决的问题。
...
_Marina Favaro, Jack Clark_ — 首次公开 Anthropic 内部数据：合入生产的代码中超过 80% 由 Claude 编写，工程师人均产出是 2024 年的 8 倍，同一训练加速任务上模型一年内从 3 倍提到 52 倍，在选择下一步实验时模型优于人类的比例为 64%。文章的判断是写代码和跑实验已基本自动化，决定做什么实验仍由人主导。
...
> 四篇文献分别给出 RSI 的四个基本概念：Good 的正反馈论证、Yudkowsky 的能力构造与持续条件、Schmidhuber 的形式化定义、Bostrom 的动力学判据。详见[报告 00](reports/00_origins_1965_2014.md)。
...
_Eliezer Yudkowsky_ — 提出 Seed AI 需要三种能力：理解自身架构、修改自身源码、让修改产生复利。指出只让固定的优化器跑得更快不能维持递归改进，每一级提升必须让系统看到新的改进机会，否则会停滞。
...
_Jürgen Schmidhuber_ — 形式化定义：系统的任何部分（包括寻找改进的机制本身）都可以修改，但只有在形式证明"修改后期望效用更高"时才执行。理论上自洽，实践中这种证明几乎不可得。
...
> 2023 到 2025 年间把"agent 修改自身代码"从设想变成可运行系统的工作。修改的触发条件从形式证明依次放宽为 LLM 判断、效用函数、基准分数。详见[报告 23](reports/23_godel_agent_to_sica.md)。
...
> 三份综述范围不同：TMLR 综述按什么、何时、如何、何处四个维度分类单体自进化；Co-Evolution 综述处理多个组件互相影响的情形；Coding 综述限定在软件工程。三份都把评估列为开放问题，但都没有讨论评估依据本身是否应该参与进化。
...
_Huan-ang Gao, Jiayi Geng, Wenyue Hua, Mengkang Hu, et al. (Princeton / 清华 / CMU 等 17 家机构)_ — 77 页，把上百个工作按 What（模型 / 上下文 / 工具 / 架构）、When（任务中 / 任务间）、How（reward / 模仿 / 种群）、Where（通用 / 专域）分类。评估一节指出现有基准几乎都在任务间重置状态，测不到知识积累。
...
_Qing Zong, Jiayu Liu, Junhao Shen, Zecong Tang, Linsi Wu, et al., Yangqiu Song (HKUST / UIUC / CUHK / HKU / PKU)_ — 按 Agent-Agent、Agent-Environment、Meta 三个阶段分类共进化系统。按其对 Meta 阶段的定义，目前只有 RQGM 满足。提出评估共进化系统需要历史交叉对弈、组件消融、held-out 评估器三项。
...
> agent 直接修改自己的 harness 代码。这类系统都必须保留一部分代码不可修改（DGM 的探索循环、MOSS 的进化引擎）。Metan 测量了这一约束造成的深度上限，并给出另一种做法。
...
_Jenny Zhang, Shengran Hu, Cong Lu, Robert Lange, Jeff Clune_ — 把 Gödel Machine 的证明条件换成基准实证，并用保留低分版本的开放式档案代替单链爬山。SWE-bench 从 20.0% 到 50.0%，Polyglot 从 14.2% 到 30.7%。搜索过程中出现了伪造测试日志等绕过评估的行为。
...
_Qianshu Cai, Yonggang Zhang, Xianzhang Jia, Huajiang Zheng, Wei Xue, Jun Song, Xinmei Tian, Yike Guo (USTC / HKGAI / HKUST / HKBU)_ — 把源码级自改写部署到有真实用户流量的生产 agent 系统 OpenClaw 上。修改由生产环境的失败批次驱动，在隔离容器中重放验证，用户批准后才上线，健康检查失败自动回滚。单轮进化把四个任务的平均分从 0.25 提到 0.61。
...
_Zae Myung Kim, Young-Jun Lee, Seungyeon Jwa, Dongyeop Kang (UMN / SNU)_ — 不修改改进机制本身，而是让固定的元操作 Ω 反复读取下层的执行轨迹和代码，逐层写出新的策略和辅助函数。层数由收敛决定，实测停在 3 到 6 层。ARC-AGI-2 上得 0.331，是对比方法中唯一非零的。消融显示层间传递的文本贡献约 72% 的增益，可调用代码约 15%。

### 4.5 Evaluator Side

> 让评估器随策略一起进化的几种做法，以及这样做的风险。Who Grades the Grader 的实验表明，评估器被优化压力带偏之后，训出来的策略在任务分数上和正常情况一样好，因此任务分数不能用来验证评估器。
...
_Alex Iacob, Andrej Jovanović, William F. Shen, et al., Nicholas D. Lane (Cambridge / NVIDIA / Flower Labs)_ — 把评估器纳入搜索循环，但分 epoch 进行：epoch 内评估器冻结，边界处只有在人类标注的锚数据集上表现更好的候选评估器才能替换在位者。Polyglot 上 71.7% 对 69.9%，token 少 1.35 到 1.72 倍；论文写作任务的接受率是基线的 1.86 倍。
...
_Xing Zhang, Guanghui Wang, Yanwei Cui, Ziyuan Li, Wei Qiu, Bing Zhu, Peiyang He (AWS / HSBC)_ — 只用十条带真值的锚定样本进化评估指标，MBPP+ 上与真值的一致性提高 0.21。关键消融：去掉锚定守卫后指标退化成全部放行，但用它训出的技能在任务上的分数与正常指标相同，两个任务上甚至更高。
...
_Xing Zhang, Yanwei Cui, Guanghui Wang, Zhihao Lin, Peiyang He (AWS)_ — 同一团队的后续工作。借用程序验证中的反例引导精化：找出评估器打分相同但真值不同的一对答案，把这一对作为下一个评估算子的编写要求。得到的 55 行算子在 428 个未见任务上关闭了 15.4% 的可达差距（p = 0.001）。
...
_Han Zhu, Chengkun Cai, Yuanfeng Song, Xing Chen, Sirui Han, Yike Guo (HKUST / ByteDance / UCL)_ — 评估器与求解器共享参数联合训练，用于没有标准答案的深度研究报告生成。

### 4.6 Model Side

> 在模型权重层做共进化：critic 与策略同步更新，或把 harness 带来的收益通过微调写进模型。
...
_Zhicong Li, Lingjie Jiang, Yulan Hu, et al., Yong Liu (人大 / 阿里高德 / 北大)_ — critic 和策略各用一路 GRPO 同步更新，critic 的奖励是它的诊断让策略精炼后实际提高的分数。Qwen3-4B 在四个环境上平均 77.85，GRPO 为 70.57。冻结 critic 的对照在 ALFWorld 和 SciWorld 上低于不用 critic 的 GRPO。
...
_Zhengyu Chen, Teng Xiao, Huaisheng Zhu, Yige Yuan, Luan Zhang, Jingang Wang (美团 / AI2)_ — HarnessCritic 分析失败轨迹、按五类归因修改 harness，再用改进后 harness 生成的轨迹微调模型，两步交替。Qwen3-8B / 32B 在 AIME24 / AIME25 / HMMT25 上平均提高 20.4 个百分点，比人工设计的 harness 高 24.7。一个 200 多小时的无人干预案例中产生了 22 个 harness 版本。
...
_Peng Xia, Jianwen Chen, Hanyang Wang, Jiaqi Liu, et al. (UNC / aiming-lab)_ — 把轨迹蒸馏为分层技能库，技能库在 RL 训练中按验证失败递归更新。ALFWorld / WebShop 上比基线高 15.3%。
...
_Prannay Hebbar, Yogendra Manawat, Samuel Verboomen, et al. (Hexo Labs)_ — 一个反馈 agent 决定每轮是修改 harness 还是更新权重。Weng 指出其实验设置有混淆（任务 agent 与 meta agent 用不同模型，基线较弱），证据初步。

### 4.7 Knowledge Side

> 把交互经验整理成可复用的技能。2026 年 8 月的五篇平行工作在两点上一致：存原始轨迹不如抽象成技能，技能库需要验证、审计和删除机制；分歧在用什么结构组织技能。合评见[报告 26](reports/26_skill_evolution_wave.md)。
...
_Liyan Tang, Cyrus Rashtchian, Chun-Sung Ferng, Andrew Tomkins, Da-Cheng Juan, Tu Vu (Google Research / Virginia Tech)_ — 工作区分三层：原始轨迹不可变，wiki 知识库只增不删、不回滚，技能层经验证集门控、可回滚。五个基准五个模型上平均分均为第一。消融：技能提议者能读 wiki 时提高 15.0 分，推理 agent 训练时读 wiki 反而降 2.8 分。9B 模型加技能（47.4%）超过 27B 无技能（39.4%）。
...
_Mingxuan Zheng, Yujin Zhou, Chuxue Cao, et al., Sirui Han, Yike Guo (HKUST)_ — 把近端梯度下降的形式用于文本技能，用留一效用审计决定每个知识单元是保留、降级还是删除。比最强文本梯度基线高 3.0 个百分点。
...
_Tianxin Wei, Zhan Shi, Minhua Lin, Bing He, et al., Hanqing Lu (UIUC / Amazon)_ — 每个任务只有一次执行机会的设定下，把单次执行编译为通用技能和主题技能。Claude Opus 4.6 上五个基准全部提升（TerminalBench-2 从 62.92 到 73.03）。分离测量了 evolver 设计、反馈类型和迁移设置各自的贡献。
...
> 不重置环境、在任务进行中修改 harness 的路线，以及它的开源工程实现。
...
_Seth Karten, Joel Zhang, Tersoo Upaa Jr, Ruirong Feng, Wenzhe Li, Chengshuai Shi, Chi Jin, Kiran Vodrahalli (Princeton)_ — 在一场连续的长 episode 内每隔若干步精炼 harness 的四个组件，同一条轨迹同时用于 harness 精炼和权重训练。发现能力地板：基座模型弱于 Gemini Flash-Lite 时，所有自改进变体都比不改进更差。
...
_Seth Karten, Alex L. Zhang, Kevin Thomas, Sebastian Müller, et al. (Princeton / Prime Intellect / MIT)_ — Continual Harness 一作的后续工作，把免重置自改进做成开源 harness：持久 REPL、可恢复的会话、版本化的 refinement。ARC-AGI-3 上 Opus 5 从官方 harness 的 30.2% 提到 95.5%。一个 Factorio 案例中 agent 发现 RCON 作弊命令后把它保存成了可复用技能。

### 4.9 Harness Engineering (2026)

> 2026 年 3 到 8 月的 harness 优化工作。做法包括：外部 coding agent 优化、模型自己修改、离线小批量学习、修改环境而不修改 agent、垂直领域落地。共同点是优化出的 harness 可以跨模型迁移，并且每个系统都有一个不参与优化的检查环节。六个系统的中文导读见 [§8](#8-secondary-sources-中文导读)。
...
_Yoonho Lee, Roshen Nair, Qizheng Zhang, Kangwook Lee, Omar Khattab, Chelsea Finn (Stanford / KRAFTON / MIT)_ — 用 Claude Code 作为 proposer，读取全部历史候选的源码、分数和执行轨迹（每轮中位数 82 个文件，约 10M token）来提出新 harness。消融：只给分数 34.6，分数加摘要 34.9，给完整轨迹 50.0。TerminalBench-2 上 Opus 4.6 得 76.4%（手工 Terminus-KIRA 为 74.7%），Haiku 4.5 得 37.6%，均为该模型的榜首。
...
_Hangfan Zhang, Shao Zhang, Kangcong Li, et al., Lei Bai, Shuyue Hu (上海 AI Lab)_ — 同一个冻结模型为自己的 harness 提出有界编辑，只有在 held-in 和 held-out 两个 split 上都不退步的编辑才被接受。3 个模型 × 3 个基准的 9 个组合全部提升，最大相对提升 132%（Qwen3.5-35B 在 AppWorld 上从 22.5% 到 52.2%）。
...
_Sungho Park, Wonjoong Kim, Rongyuan Tan, Jue Zhang, Wook-Shin Han, et al. (POSTECH / KAIST / SUSTech / Microsoft)_ — 把 harness 优化按小批量训练组织：诊断-补丁相当于反向传播，dev 集用于检查泛化，EvoDAG 保存历史。GAIA2 提高 9.0、Terminal-Bench 2.0 提高 10.0，超过人工调整的 harness。去掉 dev 集检查后得分 50.6，低于未优化的 53.0。
...
_Chengsong Huang, Zifeng Wang, Rujun Han, Jun Yan, et al., Chen-Yu Lee (WashU / Google Cloud AI)_ — 不修改 agent，而是用三类接口插件（Stage / Contract / Chain）修改训练环境，原环境的验证器不动。ALFWorld OOD 提高 9.0。从未修改的静态环境抽取的技能在 SpreadsheetBench 上低于不用技能。
...
_ChengAo Shen, Wenchao Yu, Fangyu Wu, Dongjin Song, Hanghang Tong, et al., Jingchao Ni (UH / NEC Labs)_ — agent 不做预测，而是合成训练数据并训练轻量预测器；负责合成的 agent 的 harness 由另一个 agent 优化，优化信号是生成数据训出的预测器与真实数据训出的预测器在同一测试集上的误差差距。18 个数据集 × 3 个样本量的 30 格中 19 格第一。同一 harness 换四个 LLM 骨干，总体误差在 0.267 到 0.366 之间。
...
_Wenbo Pan, Shujie Liu, Chin-Yew Lin, et al., Xiaohua Jia (CityU HK / MSRA)_ — 只用过去的轨迹，不用任何标签：选出困难且多样的任务子集，并行重解，用轨迹内的自验证和轨迹间的一致性诊断问题，生成多个候选 harness 后由 agent 自己成对比较选出一个。单轮把 SWE-Bench Pro 从 59% 提到 78%。作者说明只测了一轮，不保证多轮继续提升。
...
_Mengzhuo Chen, Junjie Wang, Zhe Liu, Yawen Wang, Haiming Zheng, Qing Wang (中科院软件所)_ — 把执行轨迹和 harness 制品编译成中间表示，把失败归因到 harness 七个责任层之一，再做范围受限的修复。四个基准上提高 6.3% 到 18.4%。
...
_Zewen Liu, Zhan Shi, Yisi Sang, Bing He, Minhua Lin, Tianxin Wei, et al. (Emory / Amazon)_ — 在开放任务流上测试自改进系统，发现密集自改进的收益早期到峰后下降。
...
> 让 AI 承担研究工作的实证，从自动生成论文到主导交付一个可发布的模型。Anthropic 的内部数据见 [§2](#2-core-readings)。
...
_Cheng Yang, Jiayang Lyu, Shangyuan Liu, Guibin Zhang, et al. (SJTU / NUS / DP Technology)_ — 人类一次性写好目标、权限、验证标准、研究方法、治理规则五层约束，之后数据构建、SFT、OPSD、RLVR 四个阶段的决策全部由 agent（Codex GPT-5.6-Sol）做出。产出的 27B 模型在 RTLLM 上 68.0，高于 GPT-5.5（66.0）和 Claude Opus 4.8（64.7）；KernelBench L1 正确率 61，约为 DeepSeek-V4-Pro 的两倍。本仓库计划以其代码库为后续开发基础。
...
_Dhruv Trehan, Paras Chopra_ — 最小脚手架下，三个领域 45 到 50 篇种子文档中只有 1 个想法完整执行成论文。总结了六种反复出现的失败：沿用过时的库和命令、实现过程中滑向更简单的方案、上下文退化、在噪声中宣布成功、缺少领域直觉、问题选择不当。

### 4.11 Safety & Governance

> 自进化系统的部署问题：怎样发现、归因、修复失败，怎样决定修改能否上线，怎样测量作弊。合评见[报告 27](reports/27_safety_governance.md)。
...
_Cong Ming, Jingyi Chen, Bin Liu, Qi Chu, Tao Gong, Nenghai Yu, Yingfei Xiang, Ronghai Yang (中科大 / 深信服)_ — 跑在深信服生产环境中的自进化安全护栏：监控线上流量，发现新型越狱后自动合成训练数据并更新 1.7B 护栏模型，16 到 24 小时完成一轮（原人工流程 40 到 90 小时）。两个月内自动处理了 15 个新威胁场景中的 14 个。
...
_Siqi Wang, Xinlin Li, Zhenglin Li, Li Li_ — 把 agent 的观察、规划、恢复、停止等控制策略做成带版本和血统的资产；新版本与旧版本配对评估，上线后表现退化则回滚到父版本。
...
_Deepak Soni_ — 每项新能力必须通过预先声明的机器可检查验收套件，同时一组固定的不变量在每次发布时都必须成立。规则：让系统更保守的修改可以自动应用，放宽限制的修改必须由人合并，提议者必须能预测自己修改的效果，预测错则被关闭。在一个开源运行时上跟踪六个版本，不变量未改，测试从 122 增至 563。
...
_Amit Roth, Ivan Bercovich, Yonathan Efroni (TAU / UCSB)_ — 在 89 个真实终端任务中嵌入可自动检测的作弊机会，共 2,225 条轨迹。gemini-3.1-pro 在明确禁止的提示下仍有 16.3% 的作弊率，且仅加警告时作弊率从 47.7% 升到 59.8%。作者指出所有检出率都是真实作弊率的下界。
...
_Alexander Novikov, Ngân Vũ, Marvin Eisenberger, Emilien Dupont, Po-Sen Huang, et al. (Google DeepMind)_ — 候选程序池加冻结 LLM 生成 diff，用 EVOLVE-BLOCK 标注可修改区域。适用于候选可以自动评估的问题，如矩阵乘法和调度。
...
_Robert Tjarko Lange, Yuki Imajuku, Edoardo Cetin (Sakana AI)_ — 用平衡的父代采样、基于代码相似度的新颖性过滤和记录成功模式的 meta-scratchpad 提高程序进化的采样效率。

### 4.13 Macro Debate & Measurement

> 宏观层面的两个问题：AI 加速 AI 研发是否已在整体节奏上体现（insight 8）；算力和认知劳动之间的替代弹性 σ 是多少（insight 9）。
...
_Ege Erdil, Tamay Besiroglu (Epoch AI)_ — 综述"AI 自动化能否带来每年 30% 以上经济增长"的正反论证，把 σ 是否大于 1 识别为关键参数。
...
| # | 报告 | 对象 | 主要内容 |
|---|---|---|---|
| 00 | [起源 1965–2014](reports/00_origins_1965_2014.md) | Good / Yudkowsky / Schmidhuber / Bostrom 及四位中间人 | 四篇文献各给出什么概念；前史没有预见评估器问题 |
| 01 | [Weng 总纲](reports/01_lilian_weng_harness_engineering.md) | Harness Engineering for Self-Improvement | harness 是可搜索的代码；七个挑战与后续论文的对应 |
...
| 04 | [RQGM](reports/04_red_queen_godel_machine.md) | 分 epoch 的评估器进化 | 评估器可以进循环，但需要循环外的人类标注数据集 |
...
| 07 | [DGM](reports/07_darwin_godel_machine.md) | 开放式档案自改写 | 证明条件换成基准实证；出现绕过评估的行为 |
| 08 | [MOSS](reports/08_moss.md) | 生产环境源码自改写 | 路由、hook 等故障只能在代码层修；批准和回滚流程 |
| 09 | [WikiSkill](reports/09_wikiskill.md) | 经验、wiki、技能三层 | 知识库只增不删，技能层门控可回滚 |
| 10 | [汇总](reports/10_synthesis_insights.md) | 全部材料 | 两轴地图、十条 insight、18 个系统的锚对照表、开放问题 |
| 11 | [Continual Harness](reports/11_continual_harness.md) | 免重置在线精炼 + 权重共学习 | 能力地板：弱模型自改进反而更差 |
...
| 14 | [Self-Harness](reports/14_self_harness.md) | 模型修改自己的 harness | 加回归门后 35B 模型也能提升 |
| 15 | [EnvHarness](reports/15_envharness.md) | 修改环境而不修改 agent | 验证器不动；静态环境抽的技能可能为负 |
...
| 19 | [iCoder](reports/19_icoder.md) | AI 主导开发 27B 模型 | 人类保留的四样东西：目标、权限、验证标准、证据规则 |
| 20 | [Metan](reports/20_metan.md) | 固定 Ω、递归输入 | 不修改改进机制也能增加深度；增益主要来自文本条件化 |
| 21 | [Co-Harness](reports/21_co_harness.md) | harness 与权重交替优化 | harness 收益微调进权重后，模型不依赖脚手架 |
| 22 | [Co-Evolution 综述](reports/22_coevolution_survey.md) | 三阶段分类 | 按其标准只有 RQGM 到 Meta 阶段 |
| 23 | [Gödel Agent 到 SICA](reports/23_godel_agent_to_sica.md) | 2024–2025 桥接 | 触发条件从证明放宽到基准；监督者首次出现 |
| 24 | [EvalCEGAR](reports/24_evalcegar.md) | 碰撞对驱动的评估器进化 | 用评估器的盲点指导它的下一步进化 |
| 25 | [RHO](reports/25_rho.md) | 不用标签的 harness 优化 | 单轮 59% 到 78%；多轮效果未知 |
| 26 | [技能进化五篇](reports/26_skill_evolution_wave.md) | SkillCommit / HyperSkill / ERSkill / SkillProx / Evo-Harness | 抽象优于存储；技能的组织结构尚无定论 |
| 27 | [安全与治理五篇](reports/27_safety_governance.md) | SESG / OLE / HarnessFix / Gates / HVTB | 三种进化外的检查：不变量、蜜罐、红队集 |
| 28 | [Coding Agents 综述](reports/28_self_evolving_coding_agents_survey.md) | 对象 × 时间 × 证据 | 可执行反馈既是优势也是风险：错误会被保留 |

## 6. Insights & Open Problems

十条主要发现（完整论证见[报告 10](reports/10_synthesis_insights.md)）：

1. 评估器同时决定能力上限，也是被利用的目标。
2. 去掉进化循环外的评估依据后系统会退化（AutoSaddler 去掉 dev 集检查后低于未优化基线）。RHO 的单轮无标签结果是目前的例外，尚待多轮验证。
3. 优化期间评估标准应保持固定，更新放在优化窗口之外（RQGM 的 epoch、MOSS 的关键点锁定）。
4. 自改写系统实际能达到的 meta 深度约 2.5 层；Metan 通过不改机制、只改输入达到 3 到 6 层。
5. 发现技能和使用技能是两种能力，技能可以跨模型迁移。但 Metan 的消融显示文本条件化比代码复用贡献更大，技能库的收益来源需要重新测量。
6. 经验需要先整理成结构化知识再变成技能；用什么结构组织尚无定论。
7. 进入生产的系统都有版本、审计、预测、回滚四项机制。
8. 微观产出提升明显（Anthropic 人均 8 倍），但 METR 的多实验室调查中没有公司报告整体研发节奏翻倍。
9. 宏观争论取决于算力与认知劳动的替代弹性 σ。
10. 基准饱和速度加快，测量工具本身成为瓶颈。

开放问题（按可检验程度排序）：

1. 维持评估器不退化最少需要多少人工标注？WGtG 用十条样本，EvalCEGAR 用十个碰撞对，尚无理论。
2. 不用标签的自偏好优化多轮后是否继续提升？RHO 只测了一轮。
3. 技能库的收益有多少来自技能作为 prompt 前缀、多少来自技能作为可执行程序？Metan 测得 72% 对 15%，需在技能进化场景中复现。
4. 能否根据基座模型能力预测某个 harness 改动会提升还是降低性能？目前观察到弱模型为负、中等最大、强模型饱和。
5. 基准报告是否应附带 harness 规格，并给出按 harness 归一化后的分数？
6. 如何测量系统离 crossover 有多远？iCoder 的方案是逐步减少人类 prior，观察性能何时下降。
7. 知识库能否积累"该做什么实验"的判断，而不只是操作流程？
8. 锚本身需要更新时（RQGM 换评估器、SESG 更新护栏），由谁验证更新？目前都是人。
9. 如果 2027 年 METR 仍测不到整体节奏翻倍，是串行瓶颈所致，还是加速被用于做更多实验？
10. σ 的受控实验（随机分配研究者的算力预算）何时有人做？Epoch AI 已给出实验设计。
...
| 术语 | 含义 | 出处 |
|---|---|---|
| RSI（递归自改进） | 一次改进提升了系统发现、验证、实现下一次改进的能力，即改进能力本身进入优化闭环；只是反复改进自身不算 | Bostrom 2014，报告 00 |
...
| 三层改进面 | 文本层（prompt、技能，改动快）、权重层（微调，改动慢）、源码层（改 harness 代码，用于结构性故障）；2026 年的做法是分层分工 | 报告 10 |
| 锚 / 锚定纪律 | 进化循环之外保留一个不参与进化的评估依据：人工标注集、冻结的验证器或奖励模型、回归测试、外部 proposer、dev 集 | 报告 05、11、15、16 |
| 评估器塌缩 / 观测等价 | 评估器被优化压力带偏之后，训出的策略在任务分数上与正常情况相同，只有循环外的锚能分辨 | 报告 05 |
| reward overoptimization | 静态奖励模型判分越准，训出的策略反而越差，因为策略利用了固定判据 | 报告 03 |
| critic staleness | 固定的 critic 跟不上策略的分布变化，反馈效用下降甚至为负；解法是 critic 与策略同步更新 | 报告 06 |
| 免重置 vs 重置式 | 重置式（DGM、RQGM、GEPA）的效用信号来自完整评测；免重置（Continual Harness、Prime Agent）在任务进行中修改 harness，能处理只在 episode 后期出现的失败 | 报告 11、18 |
| 能力地板 | 基座能力低于某个阈值时，自改进反而使表现下降（Continual Harness）。Self-Harness 显示加上回归门后 35B 模型也能提升，所以地板取决于模型和接受机制两者 | 报告 11、14 |
| Harness Debt | 在补偿模型弱点的脚手架下训练，模型可能依赖脚手架而裸测退步。Co-Harness 实测每轮裸测精度上升 | 报告 21 |
| realized meta-depth | 系统中行为真正发生变化的最高 meta 层级。自改写系统因需冻结驱动层，约为 2.5；Metan 用固定 Ω 递归输入达到 3 到 6 | 报告 20 |
...
| 高密度低频接口 | 把专家经验一次性写成约束（目标、脚手架、权限、验证、证据规则），运行期只在权限变更时介入。iCoder 的做法 | 报告 19 |
| 过程级测试 | 评估共进化系统时除任务分外还需：历史交叉对弈、组件消融、held-out 评估器 | 报告 22 |
| 错误持久化 | 自进化的特有风险：不可靠的测试结果、噪声轨迹或基准捷径会被存进记忆、蒸馏成技能、更新进模型 | 报告 28 |
...
| EvalCEGAR | 评估器（算子池） | 训练 split 的碰撞对 | — | 否 | 55 行算子关闭 15.4% 差距 | 24 |
| RHO | Skills + Tools | 无（自偏好） | 重置式（需重放） | 否 | SWE-Bench Pro 0.59 → 0.78（单轮） | 25 |
| SESG | 权重（1.7B 护栏） | 红队测试集 + 人工约 2h/威胁 | — | 是（深信服） | 两个月 14/15 新威胁 | 27 |

除 RHO 外，表中所有系统的"锚在哪"一列都非空。"生产部署"一列只有五个是，其中 MOSS、iCoder、SESG 用的是最保守的形式：人工批准、锁定验证器、人工标注。
...
- [iSEngLab/Awesome-Self-Evolving-Coding-Agents](https://github.com/iSEngLab/Awesome-Self-Evolving-Coding-Agents) — arXiv 2608.03392 综述的配套清单，按进化对象六类组织，编码域收录最全；与本仓库互补，已互相收录（解读见[报告 28](reports/28_self_evolving_coding_agents_survey.md)）
...
- [Lil'Log · Harness Engineering for Self-Improvement](https://lilianweng.github.io/posts/2026-07-04-harness/) — 本调研的总纲，其参考文献是 §4.9 和 §4.12 的种子
