module design(input wire [63:0] req, output wire valid, output wire [5:0] index, output wire [63:0] grant);
  ...
  wire [15:0] group_grant;

  assign valid = |req;

  // 4-bit group valid signals
  assign gv[0] = |req[3:0];
  assign gv[1] = |req[7:4];
  ...
  // Hierarchical group selection: first level among 4-bit groups
  // group_sel[3:0] indicates which 4-group block (0-3) contains highest priority group
  assign group_valid[0] = |gv[3:0];
  assign group_valid[1] = |gv[7:4];
  assign group_valid[2] = |gv[11:8];
  assign group_valid[3] = |gv[15:12];

  assign group_sel[0] = group_valid[0] & ~(|group_valid[3:1]);
  assign group_sel[1] = group_valid[1] & ~(|group_valid[3:2]);
  assign group_sel[2] = group_valid[2] & ~(|group_valid[3:3]);
  assign group_sel[3] = group_valid[3];

  // Second level: within each 4-group block, select highest group
  // win[15:0] is one-hot among groups
  assign win[0] = gv[0] & group_sel[0] & ~(|gv[3:1]);
  assign win[1] = gv[1] & group_sel[0] & ~(|gv[3:2]);
  assign win[2] = gv[2] & group_sel[0] & ~(|gv[3:3]);
  assign win[3] = gv[3] & group_sel[0];

  assign win[4] = gv[4] & group_sel[1] & ~(|gv[7:5]);
  assign win[5] = gv[5] & group_sel[1] & ~(|gv[7:6]);
  assign win[6] = gv[6] & group_sel[1] & ~(|gv[7:7]);
  assign win[7] = gv[7] & group_sel[1];

  assign win[8] = gv[8] & group_sel[2] & ~(|gv[11:9]);
  assign win[9] = gv[9] & group_sel[2] & ~(|gv[11:10]);
  assign win[10] = gv[10] & group_sel[2] & ~(|gv[11:11]);
  assign win[11] = gv[11] & group_sel[2];

  assign win[12] = gv[12] & group_sel[3] & ~(|gv[15:13]);
  assign win[13] = gv[13] & group_sel[3] & ~(|gv[15:14]);
  assign win[14] = gv[14] & group_sel[3] & ~(|gv[15:15]);
  assign win[15] = gv[15] & group_sel[3];

  // Generate grant bits: each grant bit is req bit AND win of its group
  // For group 0
  assign grant[0] = ~(|req[63:1]); // special case: no request -> grant[0]=1
  ...
  // Index encoding from grant one-hot
  assign index[0] = grant[1] | grant[3] | grant[5] | grant[7] | grant[9] | grant[11] | grant[13] | grant[15] | grant[17] | grant[19] | grant[21] | grant[23] | grant[25] | grant[27] | grant[29] | grant[31] | grant[33] | grant[35] | grant[37] | grant[39] | grant[41] | grant[43] | grant[45] | grant[47] | grant[49] | grant[51] | grant[53] | grant[55] | grant[57] | grant[59] | grant[61] | grant[63];
  assign index[1] = grant[2] | grant[3] | grant[6] | grant[7] | grant[10] | grant[11] | grant[14] | grant[15] | grant[18] | grant[19] | grant[22] | grant[23] | grant[26] | grant[27] | grant[30] | grant[31] | grant[34] | grant[35] | grant[38] | grant[39] | grant[42] | grant[43] | grant[46] | grant[47] | grant[50] | grant[51] | grant[54] | grant[55] | grant[58] | grant[59] | grant[62] | grant[63];
